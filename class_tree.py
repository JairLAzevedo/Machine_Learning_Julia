...
class DecisionTree:
    ...
    def _grow_tree(self, X,y, depth=0):
        ...
        feat_indx = np.random.choice(n_features, self.n_feats, replace=False)    
    
        best_feat, best_thresh = self._best_criteria(X,y,feat_indx)
        left_indx, right_indx = self._split(X[:,best_feat], best_thresh)
        left = self._grow_tree(X[left_indx,:], y[left_indx], depth+1)
        right = self._grow_tree(X[right_indx,:], y[right_indx], depth+1)
        return Node(best_feat,best_thresh, left, right)
    
    def _best_criteria(self,X,y,feat_indx):
        best_gain = -1
    ...
    def _information_gain(self,y,X_column, split_thresh):
        ...
        left_indx, right_indx = self._split(X_column, split_thresh)
